...
- **施設端末の持ち出し検知**: 施設の設定範囲外へ持ち出されたら管理者へLINE通知（2026-09-12 追加）。判定はサーバ側 `/devmon`、検知は専用アプリ `devicewatch/`。詳細は下記「施設端末の持ち出し検知」節。
...
| 端末API | `api/device-report.js` | 監視アプリ専用（`register` / `report`） |
...
端末の追加報告は本来不要である。スイープは「他の端末の報告」と「管理画面を開いたとき」に走る。

★ **スイープは1インスタンスあたり45秒に1回までへ間引く**（`SWEEP_MIN_INTERVAL_MS`。
...
間引いた回は自施設1件だけを読む。
★ `/devmon/settings` は管理者しか変えないので、報告の経路では60秒だけ
ウォームインスタンス内で使い回す（無期限にしてはならない）。
...
1. **範囲外になる前から沈黙している端末は検知できない**（電源を切ってから持ち出す等）。
   OSの機能を使う以上避けられない。
   ★ 一方、**範囲外を1回でも報告したあとに沈黙した場合はスイープが確定させる**。
   ただしスイープが走るのは「他の端末が報告したとき」と「管理者がマスター管理タブを開いたとき」
   だけである（定期実行を持たない）。**端末が1台しか無い場合は、管理画面を開くまで
   通知が出ないことがある。**
2. **位置情報を切られた／アプリを削除された瞬間は検知できない。** アプリが次に起動・復帰した
   ときに権限の異常を報告する。それも無ければ受信途絶（既定24時間）または
   「位置を判定できていません」として現れる。
3. **受信途絶の検知に定期実行（cron）を持たない。** 端末の報告時と管理画面を開いた時点で評価する。
   したがって「電源を切られた端末」は、他の端末の報告か管理画面の表示があるまで通知されないことがある。
   ★ 定期実行にするには GitHub Secrets か Vercel 環境変数の追加が必要で、
   Agent からは変更しない（本書の禁止事項）。必要なら人へ依頼する。

...
/mileage/places/{placeId}                        {name, facility, order, active, createdAt, updatedAt}
...
| `a`（管理者） | `/api/auth/admin` | マスタ・単価・利用ON/OFF・承認・月次確定・要確認/差異の確認（`autoCheck`）。`S.adminSessionValid` 必須 |
